...
```{r}
snafun::is_connected(yes_graph)
snafun::g_density(yes_graph)
```
...
```{r}
all(rownames(adj_matrix) == node_df$country_text_id)
```


```{r}
model1 <- GERGM::gergm(adj_matrix2 ~ edges + triangles + nodecov("v2x_polyarchy"),
...
network::set.edge.attribute(yes_net, "weight", as.vector(adj_matrix[adj_matrix > 0]))
network::set.vertex.attribute(yes_net, "democracy_score", node_df$v2x_polyarchy)
```
...
network::set.edge.attribute(agree_net, "weight", as.vector(adj_matrix[adj_matrix > 0]))
...
```{r}
model3 <- ergm(agree_net ~ edges + nodecov("democracy_score"),
               response = "weight",      # the edge attribute
               reference = ~Poisson)     # Poisson distribution for counts
...
```{r}
search.ergmTerms(keywords=c('valued','undirected'))
```
Terms we can try:
 * *transitiveweights(twopath="min", combine="max", affect="min") (valued)* that measures triadic closure in a valued network (i.e., how the weights of two edges influence the likelihood of forming a third edge in a triangle)
 * *sociality(attr=NULL, base=1, levels=NULL, nodes=-1, form="sum") (valued)* how a node's total weight (the sum of its edge weights) influences the structure of the network. If set to `NULL` it models the general sociality of nodes (their tendency to form edges based on their total edge weights). If set to specific attribute, it models how that attribute influences the node's activity/popularity.
 
 
```{r}
model4 <- ergm(agree_net ~ edges + transitiveweights(twopath="min", 
                                                     combine="max",
                                                     affect="min") + 

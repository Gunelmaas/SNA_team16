...
`gwdsp` captures the tendency for two nodes to share common neighbors, but unlike `gwesp`, it doesn't require these nodes to form triangles. This can work better in dense networks where clustering is dominated by shared neighbors rather than transitive triangles.

try `density`, `degcor`, `degreepopularity`.
...
```{r}
model_yes12 <- ergm::ergm(yes_net2 ~ edges + degree1.5 + nodecov("democracy_score"), 
                         control = ergm::control.ergm(MCMC.burnin = 1000,
                                                      MCMC.samplesize = 50000,
                                                      seed = 123,
                                                      MCMLE.maxit = 20,

...
weighted_edges <- weighted_edges %>%
  mutate(country_pair = pmin(country1, country2), 
         country_pair2 = pmax(country1, country2)) %>%
  group_by(country_pair, country_pair2) %>%
  summarise(weight = sum(weight, na.rm = TRUE), .groups = 'drop') %>%
  rename(country1 = country_pair, country2 = country_pair2)
```
...
```{r}
# round down weights
weighted_edges$weight <- floor(weighted_edges$weight)

#whether all weights are integers
all(weighted_edges$weight == as.integer(weighted_edges$weight))
```


```{r}
#network object
UNGA_network <- intergraph::asNetwork(UNGA_graph)
```

```{r}
#setting weights as edge attribute
network::set.edge.attribute(UNGA_network, "weight", weighted_edges$weight)
```


```{r}
network::get.edge.attribute(UNGA_network, "weight")
```
...
model1 <- ergm(UNGA_network ~ edges,
